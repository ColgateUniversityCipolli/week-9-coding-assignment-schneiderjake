...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \[Q = \frac{L(\{\hat{a}, \hat{\sigma}\}|\mathbf{x})}{L(\{\hat{\alpha}, \hat{\beta}\}|\mathbf{x})}=e^{\left[\mathcal{L}(\{\hat{a}, \hat{\sigma}\}|\mathbf{x}) - \mathcal{L}(\{\hat{\alpha}, \hat{\beta}\}|\mathbf{x})\right]}= 2.162312*10^(-7)\]
    Because our likelihood ratio is less than 1 that means that our Gamma distribution would have a better fit than the Weibull.
    ...
    \[Q = \frac{L(\{\hat{a}, \hat{\sigma}\}|\mathbf{x})}{L(\{\hat{\mu}, \hat{\sigma}\}|\mathbf{x})}=e^{\left[\mathcal{L}(\{\hat{a}, \hat{\sigma}\}|\mathbf{x}) - \mathcal{L}(\{\hat{\mu}, \hat{\sigma}\}|\mathbf{x})\right]}=2.371758*10^(16)\]
    Because our likelihood ratio is greater than 1 that means that our Weibull distribution would have a better fit than the Log-Normal.
  
    
    \item Compute the likelihood ratio to compare the Gamma and the Log-Normal distribution.
    Which has a better fit according to the likelhiood ratio?
    \[Q = \frac{L(\{\hat{\alpha}, \hat{\beta}\}|\mathbf{x})}{L(\{\hat{\mu}, \hat{\sigma}\}|\mathbf{x})}=e^{\left[\mathcal{L}(\{\hat{\alpha}, \hat{\beta}\}|\mathbf{x}) - \mathcal{L}(\{\hat{\mu}, \hat{\sigma}\}|\mathbf{x})\right]}=1.096862*10^(23)\]
    Because our likelihood ratio is greater than 1 that means that our Gamma distribution would have a better fit than the Log-Normal.
    
  This means that overall, our Gamma Distribution would fit our data the best. I also fit our distributions to a histogram of our data in Figure \ref{plot1}, which allows us to visually confirm that the Gamma is the best fitting distribution. 
  
  \begin{figure}[H]
  \begin{center}
  \includegraphics[width=\textwidth]{distribution.pdf}
  \caption{Distribution Plots of Owners and Renters}
  \label{plot1}
  \end{center}
\end{figure}
